...
def transform_dataset(original_dataset, output_path):
    transformed_dataset = []
    trajectory_id = 0
    ...
    try:
        ...
        print(f"The dataset has been transformed and saved as '{output_path}'.")
        return os.path.basename(output_path)  # Return only the dataset name
    except Exception as e:
        print(f"Transformation failed: {e}")
        return None

def check_dataset_format(dataset_name):
    input_path = f"C:/Users/iamar/Desktop/Internship/LDPTrace/LDPTrace/data/{dataset_name}.dat"
    output_directory = "C:/Users/iamar/Desktop/Internship/LDPTrace/LDPTrace/data"

    filename, file_extension = os.path.splitext(input_path)
    output_path = os.path.join(output_directory, f"{filename}.dat")

    try:
        with open(input_path, 'r') as file:
            original_dataset = file.readlines()
    except FileNotFoundError:
        print(f"Dataset '{input_path}' not found.")
        return None

    needs_transformation = any(not line.startswith('#') and not line.startswith('>0') for line in original_dataset)

    if needs_transformation:
        transform = input(f"The dataset '{input_path}' might need transformation. Do you want to proceed with transformation? (yes/no): ")

        if transform.lower() == 'yes':
            output_dataset = transform_dataset(original_dataset, output_path)
            if output_dataset:
                original_rename_path = f"original_{dataset_name}.dat"
                if os.path.exists(original_rename_path):
                    print(f"File 'original_{dataset_name}.dat' already exists. Removing the existing file...")
                    os.remove(original_rename_path)  # Remove the existing file
                os.rename(input_path, original_rename_path)
                os.rename(output_path, f"{dataset_name}.dat")
                return output_dataset
            else:
                return None
        else:
            print(f"No transformation performed for the dataset '{input_path}'.")
            return None

    else:
        print(f"No transformation needed for the dataset '{input_path}'.")
        return os.path.basename(input_path)  # Return only the dataset name

if __name__ == "__main__":
    dataset_name = (args.dataset)  # Replace with your dataset name
    transformed_dataset = check_dataset_format(dataset_name)
    if transformed_dataset:
        print(f"Data processing completed. Transformed dataset: '{transformed_dataset}'")
    else:
    ...
    print(f'Density Error calculated is: {density_error}')

    print('\n2. Experiment: Hotspot Query Error')
    hotspot_ndcg = experiment.calculate_hotspot_ndcg(orig_density, syn_density)
    print(f'Hotspot Query Error calculated is: {1-hotspot_ndcg}')
    # Query AvRE
    print('\n3. Experiment: Query AvRE')
    ...
    query_error = experiment.calculate_point_query(orig_sampled_trajectories,
                                                ...
                                                queries)
    print(f'Point Query AvRE calculated is: {query_error}')

    # Location coverage Kendall-tau
    print('\n4. Experiment: Kendall-tau')
    kendall_tau = experiment.calculate_coverage_kendall_tau(orig_grid_trajectories,
                                                            synthetic_grid_trajectories,
                                                            grid_map)
    print(f'Kendall_tau calculated is:{kendall_tau}')

    # Trip error
    print('\n5. Experiment: Trip error')
    ...
    print(f'Trip error calculated is: {trip_error}')

    # Diameter error
    print('\n6. Experiment: Diameter error')
    diameter_error = experiment.calculate_diameter_error(orig_trajectories, synthetic_trajectories,
                                                        multi=args.multiprocessing)
    print(f'Diameter error calculated is: {diameter_error}')

    # Length error
    print('\n7. Experiment: Length error')
    length_error = experiment.calculate_length_error(orig_trajectories, synthetic_trajectories)
    print(f'Length error calculated is: {length_error}')

    # Pattern mining errors
    print('\n8. Experiment: Pattern mining errors')
